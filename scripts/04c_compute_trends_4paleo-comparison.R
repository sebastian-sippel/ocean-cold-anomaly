...
# 01a. Ocean Regions from Cowtan-hybrid36 HadSST3:
# ------------------------------------------------------------------------------------
{
  {
    hybrid_36m_IndianOcean_ <- hybrid_36m_WAtlantic_ <- hybrid_36m_WPacific_ <- hybrid_36m_EPacific_ <- list()
    ...
    for (mon in 1:12) {
      ...
      grid.ix = which(lat < 20 & lat > -15 & lon > 40 & lon < 100)
      w = areaw[grid.ix] * (1-land_fraction[grid.ix]) / sum(areaw[grid.ix] * (1-land_fraction[grid.ix]))  
      hybrid_36m_IndianOcean_[[mon]] = sapply(X = 1:dim(hybrid_36m_)[1], FUN=function(i) sapply(1:2, FUN=function(ens) weighted.mean(x = (hybrid_36m_[i,grid.ix]), w = w, na.rm = T)))
      
      # Western Pacific (25◦N–25◦S, 110–155◦E)
      grid.ix = which(lat < 25 & lat > -25 & lon > 110 & lon < 155)
      w = areaw[grid.ix] * (1-land_fraction[grid.ix]) / sum(areaw[grid.ix] * (1-land_fraction[grid.ix]))
      hybrid_36m_WPacific_[[mon]] = sapply(X = 1:dim(hybrid_36m_)[1], FUN=function(i) sapply(1:2, FUN=function(ens) weighted.mean(x = (hybrid_36m_[i,grid.ix]), w = w, na.rm = T)))
      
      # Eastern Pacific (10◦N–10◦S, 175◦E–85◦W)
      grid.ix = which(lat < 10 & lat > -10 & lon > 175 & lon < 275)
      # land_fraction1 = land_fraction; land_fraction1[grid.ix] = 5; image.plot(matrix(land_fraction1, 72, 36)); 
      w = areaw[grid.ix] * (1-land_fraction[grid.ix]) / sum(areaw[grid.ix] * (1-land_fraction[grid.ix]))
      hybrid_36m_EPacific_[[mon]] = sapply(X = 1:dim(hybrid_36m_)[1], FUN=function(i) sapply(1:2, FUN=function(ens) weighted.mean(x = (hybrid_36m_[i,grid.ix]), w = w, na.rm = T)))
      
      # Western Atlantic (15–30◦N, 60–90◦W)
      grid.ix = which(lat < 30 & lat > 15 & lon > 270 & lon < 300)
      # land_fraction1 = land_fraction; land_fraction1[grid.ix] = 5; image.plot(matrix(land_fraction1, 72, 36)); 
      w = areaw[grid.ix] * (1-land_fraction[grid.ix]) / sum(areaw[grid.ix] * (1-land_fraction[grid.ix]))
      hybrid_36m_WAtlantic_[[mon]] = sapply(X = 1:dim(hybrid_36m_)[1], FUN=function(i) sapply(1:2, FUN=function(ens) weighted.mean(x = (hybrid_36m_[i,grid.ix]), w = w, na.rm = T)))
    }
    
    ## Process into ANNUAL x ENSEMBLE format:
    hybrid_36m_IndianOcean_ann_ens = get.ens.avg_Apr_Mar(x = hybrid_36m_IndianOcean_, nrow = 2, ncol = 167)
    hybrid_36m_WAtlantic_ann_ens = get.ens.avg_Apr_Mar(x = hybrid_36m_WAtlantic_, nrow = 2, ncol = 167)
    hybrid_36m_WPacific_ann_ens = get.ens.avg_Apr_Mar(x = hybrid_36m_WPacific_, nrow = 2, ncol = 167)
    hybrid_36m_EPacific_ann_ens = get.ens.avg_Apr_Mar(x = hybrid_36m_EPacific_, nrow = 2, ncol = 167)
  }
  # get Tropics full ensemble with the same weights as Abram et al:
  # str(IndianOcean_ann_ens)
  hybrid_36m_Tropics_ann_ens = c(sapply(X = 1:1, FUN = function(ix) {
    return(sapply(X = 1:166, FUN=function(i) {
      weighted.mean(x = c(hybrid_36m_IndianOcean_ann_ens[ix,i], hybrid_36m_WAtlantic_ann_ens[ix,i], hybrid_36m_WPacific_ann_ens[ix,i]), w = c(w_IOcean, w_WAtlantic, w_WPacific), na.rm = T)
    }))
  }))
...
# ------------------------------------------------------------------------------------
{
    ...
    BEST_Indian_Ocean.trend = get.trend(x = BEST_Indian_Ocean, trend.years = trend.years)
    ...
    BEST_WPacific.trend = get.trend(x = BEST_WPacific, trend.years = trend.years)
    ...
    BEST_EPacific.trend = get.trend(x = BEST_EPacific, trend.years = trend.years)
    ...
    BEST_WAtlantic.trend = get.trend(x = BEST_WAtlantic, trend.years = trend.years)
...
## Correction of HadSST4 dataset with hybrid36-Cowtan et al.:
...
# Calculate trends in OBS from different instrumental reconstructions:
OBS.GMST_tos.trends = apply(X = OBS.tos_$GMST$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.GMST_tas_land.trends = apply(X = OBS.tas_land_$GMST$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)

OBS.GMSST_tos.trends = apply(X = OBS.tos_$GMSST$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.GMSST_tos.trends_q = apply(X = OBS.GMSST_tos.trends, MARGIN = 1, FUN=function(x) quantile(x, probs=probs))

OBS.GMSST_tas_land.trends = apply(X = OBS.tas_land_$GMSST$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.GMSST_tas_land.trends_q = apply(X = OBS.GMSST_tas_land.trends, MARGIN = 1, FUN=function(x) quantile(x, probs=probs))

OBS.GMLSAT_tos.trends = apply(X = OBS.tos_$GMLSAT_NI$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.GMLSAT_tas_land.trends = apply(X = OBS.tas_land_$GMLSAT_NI$ann$mod_p1_min, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
...
OBS.GMSST_hybrid36cor.trends = apply(X = OBS.tos_GMSST_cor, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
...
OBS.Tropics_tas_land.trends = apply(X = Tropics_tas_land_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.Tropics_tos.trends = apply(X = Tropics_tos_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)

OBS.WAtlantic_tas_land.trends = apply(X = WAtlantic_tas_land_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.WAtlantic_tos.trends = apply(X = WAtlantic_tos_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)

OBS.WPacific_tas_land.trends = apply(X = WPacific_tas_land_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
OBS.WPacific_tos.trends = apply(X = WPacific_tos_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)

OBS.IndianOcean_tas_land.trends = apply(X = IndianOcean_tas_land_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
...
HadSST4.WAtlantic.trends = apply(X = WAtlantic_ann_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
HadSST4.WPacific.trends = apply(X = WPacific_ann_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
HadSST4.IndianOcean.trends = apply(X = IndianOcean_ann_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)
HadSST4.EPacific.trends = apply(X = EPacific_ann_ens, MARGIN = 1, FUN=get.trend, trend.years = trend.years, years = 1850:2020)


## Trends in SST datasets:
# Calculate missing trends:
HadSST4.trends = get.trend(x = HadSST4.global.annual$Anomaly, trend.years = trend.years, years = 1850:2020)
COBE_SST2.trends = get.trend(x = COBE_SST2.annual$Anomaly, trend.years = trend.years, years = COBE_SST2.annual$Year)
ERSSTv5.trends = get.trend(x = ERSSTv5.annual$Anomaly, trend.years = trend.years, years = ERSSTv5.annual$Year)



hybrid_36m_Tropics_ann.trends = get.trend(x = hybrid_36m_Tropics_ann_ens, trend.years = trend.years, years = 1850:2015)
hybrid_36m_IndianOcean_ann.trends = get.trend(x = hybrid_36m_IndianOcean_ann_ens[1,], trend.years = trend.years, years = 1850:2015)
hybrid_36m_WAtlantic_ann.trends = get.trend(x = hybrid_36m_WAtlantic_ann_ens[1,], trend.years = trend.years, years = 1850:2015)
hybrid_36m_WPacific_ann.trends = get.trend(x = hybrid_36m_WPacific_ann_ens[1,], trend.years = trend.years, years = 1850:2015)
...
# Ocean2k
ocean2k.trends = get.trend(x = ocean2k_$mod_p1_min_50, trend.years = trend.years, years = 1850:2001)

## Tierney et al. 2015:
Tierney_WAtlantic = t(sapply(X = 1:70, FUN=function(i) get.trend(x = Tierney_regions$atlantic[[7]][,i], trend.years =  trend.years, years = c(Tierney_regions$atlantic[[5]]))))
Tierney_IOcean = t(sapply(X = 1:98, FUN=function(i) get.trend(x = Tierney_regions$indian[[7]][,i], trend.years =  trend.years, years = c(Tierney_regions$indian[[5]]))))
Tierney_WPacific = t(sapply(X = 1:133, FUN=function(i) get.trend(x = Tierney_regions$wpacific[[7]][,i], trend.years =  trend.years, years = c(Tierney_regions$wpacific[[5]]))))
Tierney_EPacific = t(sapply(X = 1:63, FUN=function(i) get.trend(x = Tierney_regions$epacific[[7]][,i], trend.years =  trend.years, years = c(Tierney_regions$epacific[[5]]))))


### Neukom reconstructions:
neukom2019_trend_all = list(sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_BHM[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_CPS_new[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_DA[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_M08[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_OIE[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_PAI[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)),
                         sapply(2:1001, FUN=function(i) get.trend(x = neukom2019_PCR[,i], trend.years = (trend.years), years = neukom2019_BHM$Year_CE)))

# Crowley trends:
crowley2014.trend = get.trend(x = proxy.Crowley2014$`Global proxy (1782)`, trend.years = trend.years, years = proxy.Crowley2014$Year)
...

{
  ...
  for (en in 1:dim(ens.mem.un)[1]) {
    ...
    CMIP6.TMLSAT_tas_land.trends[en,] = get.trend(x = CMIP6.tas_land_all.df$ann$Yhat$TMLSAT_40S_40N[ix_tas_land], trend.years = trend.years, years = 1850:2014)
    CMIP6.TMMSAT_tos.trends[en,] = get.trend(x = CMIP6.tos_all.df$ann$Yhat$TMMSAT_40S_40N[ix_tos], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMLSAT_NI_tas_land.trends[en,] = get.trend(x = CMIP6.tas_land_all.df$ann$Yhat$GMLSAT_NI[ix_tas_land], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMSST_tos.trends[en,] = get.trend(x = CMIP6.tos_all.df$ann$Yhat$GMSST[ix_tos], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMSST_tas_land.trends[en,] = get.trend(x = CMIP6.tas_land_all.df$ann$Yhat$GMSST[ix_tas_land], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMSST_true.trends[en,] = get.trend(x = CMIP6.tos_all.df$ann$Y$GMSST[ix_tos], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMLSAT_true.trends[en,] = get.trend(x = CMIP6.tos_all.df$ann$Y$GMLSAT_NI[ix_tas_land], trend.years = trend.years, years = 1850:2014)
    CMIP6.Tropics_true.trends[en,] = get.trend(x = CMIP6.Tropics_[ix_tas_land], trend.years = trend.years, years = 1850:2014)
    CMIP6.GMST_true.trends[en,] = get.trend(x = CMIP6.tos_all.df$ann$Y$GMST_FM[ix_tos], trend.years = trend.years, years = 1850:2014)
  }
}
