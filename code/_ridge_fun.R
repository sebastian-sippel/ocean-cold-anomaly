...
weighted_ridge = function(X, y, lam = 0, weights = NULL, svd.list = NULL) {
...
fit.ridge <- function(y, X, lambda = 100,  standardize = T, weights = NULL) {
  ...
  if (standardize == T) {
    ...
    beta = weighted_ridge(X = X_sc, y = Y_sc, lam = lambda, weights = weights)$coef
...
fit.ridge_getSVD <- function(X, standardize = T, weights = NULL) {
  ...
  if (standardize == T) {
    ...
    # GET SVD of predictor space:
    root = sqrt(weights)
    X. = root * as.matrix(X_sc)
    ...
    # a0 = mu_y - colSums(beta * mu_x / sd_x) * sd_y
    # beta0 = beta / sd_x * sd_y
    return(list(svd = svd, mu_x = mu_x, sd_x = sd_x))
  } else if (standardize == F) {
    # not implemented currently!!
    return(NULL)
  }
