...
## how much does obs difference exceed CMIP6?
# plot(c(CMIP6_mod_p1_min$diff_x_y_2.5, rev(CMIP6_mod_p1_min$diff_x_y_97.5)))
mean(CMIP6_mod_p1_min$diff_x_y_2.5[51:81])
mean(CMIP6_mod_p1_min$diff_x.low_y.low_2.5[51:81])

mean(OBS_mod_p1_min$diff_x_y_50[51:81])
...
pdf(file = "02_plausibility_CMIP6.pdf", width = 8, height=10)
{
  ...
  mtext(text = "Temperature difference, GMST(ocean) minus GMST(land) [°C]", side = 2, line = 3, at = 3.5, adj = 0.5)
  ...
  
  {
    ...
    mtext(text = "Global implied SST Adjustment [°C]", side = 4, line = 3, at = 0.5, col = "blue")
  ...
  legend("bottom", c("Observational reconstruction", "Raw observations, scaled", "CMIP6 reconstruction",
                          "Implied HadSST4 adjustment", "Implied Cowtan-hybrid36 adjustment"), 
...
# REMOVE THE IMPLIED CORRECTIONS:
pdf(file = "02_plausibility_CMIP6_exp.pdf", width = 8, height=10)
{
  par(mfrow=c(1, 1), mar=c(3,5,1,5))
