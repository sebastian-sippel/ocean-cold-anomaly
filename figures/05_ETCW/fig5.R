...
pdf(file = "figures/05_ETCW/05_land_ocean_40_years.pdf", width = 8, height = 5)
    ...
    for (i in 1:4) {
      ...
      lines(x = sapply(CRUTEM5_constraint, FUN=function(x) x$mean_out), y = sapply(HadSST4_constraint, FUN=function(x) x$mean_out), col = "grey40", lty = 2, lwd = 1)
      lines(x = sapply(CRUTEM5_constraint, FUN=function(x) x$mean_out), y = sapply(hybridSST_constraint, FUN=function(x) x$mean_out), col = "brown4", lty = 2, lwd = 1)
    ...
    lines(x = get.trend(x = CRUTEM5.global.annual$Anomaly, trend.years = trend.years, years = 1850:2020)[1:4], 
          y = get.trend(x = HadSST4.global.annual$Anomaly, trend.years = trend.years, years = 1850:2020)[1:4], col = "black", lty = 2, lwd = 1)
    ...
    text(x = CRUTEM5_constraint[[2]]$mean_out, y = HadSST4_constraint[[2]]$q025 - 0.03, labels = c("1871-1910"), 
         adj = 0.5, col = "black", cex = 0.8)
    text(x = CRUTEM5_constraint[[3]]$mean_out, y = HadSST4_constraint[[3]]$q975 + 0.08, labels = c("1901-1940"), 
         adj = 0.5, col = "black", cex = 0.8)
    text(x = CRUTEM5_constraint[[4]]$mean_out + 0.06, y = HadSST4_constraint[[4]]$q025 - 0.08, labels = c("1975-2014"), 
         adj = 0.5, col = "black", cex = 0.8)
    
    
    legend("bottomright", c("Raw CRUTEM5/HadSST4",
    ...
    legend("top", c("HadSST4", "ERSSTv5", "COBE-SST2"), title = "Raw Trends", col = "blue", pch = c(8,6,4), cex = 0.7, bg = "white", inset = 0.00, ncol = 2)
    ...
    text(x = 1.6, y = -0.49, labels = c("HadSST4"), srt=90, adj = 0, col = "blue", cex = 0.75)
    ...
    text(x = 2.3, y = -0.49, labels = c("CRUTEM5"), srt=90, adj = 0, col = "darkorange", cex = 0.75)
    text(x = 2.4, y = -0.49, labels = c("CoastalHybridSST"), srt=90, adj = 0, col = "brown4", cex = 0.75)
...
# ------------------------------------------------------------------------------------
  ...
  {  
    ...
    lines(x = get.trend(x = CRUTEM5.global.annual$Anomaly, trend.years = trend.years, years = 1850:2020)[5:8], 
          y = get.trend(x = HadSST4.global.annual$Anomaly, trend.years = trend.years, years = 1850:2020)[5:8], col = "black", lty = 2, lwd = 1)
