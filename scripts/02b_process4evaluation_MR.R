...
# screen -S process4evaluation_MR
...
names.vec = c("GSAT", "GMST_FM")
file.names = c("tos_MR_predGSAT", "tos_MR_predGMST")
...
# -------------------------------------------------------
{
  ...
  OBS.tos = list(); OBS.tos[1:2] = sapply(1:2, FUN=function(i) OBS.tos[[i]] <- list() ); names(OBS.tos) = names.vec
  OBS.tos_ = list(); OBS.tos_[1:2] = sapply(1:2, FUN=function(i) OBS.tos_[[i]] <- list() ); names(OBS.tos_) = names.vec
  ...
  for (mon in 1:12) {
    ...
    ## 0.1 Load climate model monthly data & select training model indices:
    {
      ...
      load(paste("/net/h2o/climphys1/sippels/_projects/ocean-cold-anomaly/data/02_trained_models/tos_MR/tos_MR_predGSAT/1850-01.RData", sep=""))
    ...
    ## 0.2 Set up lists for respective month:
    {
      ...
      {
        for (i in 1:2) { OBS.tos[[i]][[mon]] = list() }
      }
    ...
    ## 0.3 Generate dataset with bias/uncertainty perturbations for the whole of CMIP6:
    {
      ...
      for (date.ix in 1:171) {
        ...
        load(paste("/net/h2o/climphys1/sippels/_projects/ocean-cold-anomaly/data/02_trained_models/tos_MR/", "tos_MR_predGSAT","/", format(HadSST4_calendar_[date.ix], "%Y"), "-", format(HadSST4_calendar_[date.ix], "%m"), ".RData", sep=""))
    ...
    for (date.ix in 1:171) {
      ...
      for (i in 1:length(names.vec)) {
        ...
        load(paste("/net/h2o/climphys1/sippels/_projects/ocean-cold-anomaly/data/02_trained_models/tos_MR/", file.names[i],"/", format(HadSST4_calendar_[date.ix], "%Y"), "-", format(HadSST4_calendar_[date.ix], "%m"), ".RData", sep=""))
